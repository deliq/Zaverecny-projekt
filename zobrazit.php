...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
      ...
      echo "<br> ID: ". $row["id"]. " - Meno: ". $row["meno"]. " | Priezvisko: ". $row["priezvisko"]."  | Tel. Číslo: ". $row["cislo"]. "";
      
        

      ?>

        <div class="jsonencode">
            <?php
            echo json_encode("". $row["id"] ." ". $row["meno"]." "  . $row["priezvisko"]." " . $row["cislo"].  "");
            ?>
        </div>
